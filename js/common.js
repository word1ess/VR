...
(function(){
	...
	function uLineUp_fn(q,p){
		...
		window.addEventListener('resize',function(){
			if(_this.data.opt.resize){_this.start();}
		});
		return _this;
	}
	function extend(){
		var r={},arg=arguments,l=arg.length,b=1;
		if([1,0,true,false].indexOf(arg[l-1])!=-1){b=arg[l-1];}
		for(var a=0;a<l;a++){
			for(var k in arg[a]){
				if(b&&r[k]&&typeof r[k]=='object'&&!Array.isArray(r[k])&&typeof arg[a][k]=='object'&&!Array.isArray(arg[a][k])){
	...
	function css(o,s){
		if(typeof s=='string'){
			...
			if(r){if(r.match(/^[0-9\-\.]+(px)+$/ig)||r.match(/^[0-9\-\.]+$/ig)){r=parseInt(r);}}
			return r;
		}
		if(typeof s=='object'){
			for(var n in s){
...
$(function(){
			...
			menu.find('li').each(function(){
				if(!this.submenu_handler_mouseenter){
					...
					$(this).on('mouseenter',function(){
						...
						var submenu=_this.find('ul').eq(0);
						var submenus=submenu.find('ul');
						var w=menu.width(),sw=submenu.innerWidth(),pl=0,m=0,v=0;
						if(submenu.parent().length){
							m=submenu.parent().width()-submenu.parent().find('a:first-child').width();
							pl=submenu.parent().position().left+(parseFloat(submenu.parent().css('border-left'))*2)-(m/2);
						}
						v=v+(m/2);
						if(w-pl<sw){
							v=-(sw-(w-pl));
	...
	$(window).on('resize',function(){
		...
		$('.page_header .topNavMainWrapper').height($('.page_header .topNavMainInner').height());
	...
	$(".rangeSlider").each(function(){
		...
		function setVal(data){
			...
			elem.find("input.from").css({'width':from_w+'px','font-size':fs+'px'});
		...
		elem.find("input.from").on('input paste change',function(){
			var from=$(this).val().replace(/[^0-9]+/ig,'');
		...
		elem.find("input.to").on('input paste change',function(){
			var to=$(this).val().replace(/[^0-9]+/ig,'');
			var rangeSlider=elem.find("input[name='range']").data("ionRangeSlider");
			var max_v=parseFloat(elem.find("input[name='range']").attr('data-max'));
			if(to>max_v){elem.find(".error").show();return;}
			else{elem.find(".error").hide();}
			rangeSlider.update({to: to});
		});
		elem.find('.irs-handle.from,input.from').on('mousedown touchstart',function(){
			elem.find("input.to").removeClass('active');
			elem.find("input.from").addClass('active');
		});
		elem.find('.irs-handle.to,input.to').on('mousedown touchstart',function(){
	...
	$('form a.clear').on('click',function(){
		$(this).parents('form').find(".rangeSlider").each(function(){
			var elem=$(this);
			var rangeSlider=elem.find("input[name='range']").data("ionRangeSlider");
			rangeSlider.reset();
		});		
	...
	$('.taglist').each(function(){
		$(this).find('.tag:not(.more)').each(function(i){
			this.tlq=i;
		});
	});
	$('.section-tags .dropdown2 input[name=orderby]').on('change',function(){
		var chinp=this;
		$(this).parents('.dropdown2').find('input[name=orderby]').each(function(){
			if(this.checked==true){chinp=this;}
		});
		var v=$(chinp).val();
		if(chinp.checked==false){v='';}
		...
		if($(o).find('input')[0].checked==true){
			$(o).on('mouseup',function(){
				setTimeout(function(){
					...
					$(o).off('mouseup');
					$(o).parents('.section').find('input[name=orderby]').eq(0).trigger('change');
				},50);
			});				
		}
	});
	...
	function orderTags(el,v){
		...
		if(v==''){
			tags.sort(function(a,b){return a.tlq-b.tlq;}).appendTo(con);
		}
		if(v=='appl'){
			tags.filter('.appl').sort(function(a,b){return ($(a).text())>($(b).text()) ? 1:-1}).appendTo(con);
			tags.filter('.tech').sort(function(a,b){return ($(a).text())>($(b).text()) ? 1:-1}).appendTo(con);
		}
		if(v=='tech'){
			tags.filter('.tech').sort(function(a,b){return ($(a).text())>($(b).text()) ? 1:-1}).appendTo(con);
			tags.filter('.appl').sort(function(a,b){return ($(a).text())>($(b).text()) ? 1:-1}).appendTo(con);
		}
		if(list.hasClass('active')){openTags(list);}
		else{closeTags(list);}		
	}
	$('.taglist .tag.more').on('click',function(){
		var p=$(this).parents('.taglist');
		if(!p.hasClass('active')){openTags(p);}
		else{closeTags(p);}
	});
	...
	$(window).on('resize',function(){
		...
		window.TIDTAGS=setTimeout(function(){
			$('.taglist').each(function(){
				if($(this).hasClass('active')){openTags(this);}
				else{closeTags(this);}
			});
		},300);
	});
	...
	$('.dialogbox-consultation form').on('submit',function(){
		...
		$.ajax({
			...
			dataType:'html',			
			data:data,
			beforeSend:function(){form[0].ajax=1;form.addClass('submitting');},
			error:function(){form[0].ajax=0;form.removeClass('submitting');},
			complete:function(){form[0].ajax=0;form.removeClass('submitting');},
			success:function(d){
				$('.dialogbox').fadeOut(200);
...
if (window.screen.width < 992){
		const navLinkMobile = Array.from(document.querySelectorAll('.nav-link__mobile'));
		const subMenuUl = Array.from(document.querySelectorAll('.nav-link__mobile ul'));
		const headerMenuLinks = Array.from(document.querySelectorAll('.nav-link__mobile .header__menu'));
...
// блок с ответом
const lineFaqQuestion = Array.from(document.querySelectorAll(".line_faq_question .right_block"));
if (lineFaqQuestion) {
	for (const question of lineFaqQuestion) {
		question.onclick = function(){
			// текст в блоке
			const lineFaqText = this.querySelectorAll('.whats_faq, .descript_question');
			// кнопка "узнать ответ"
			...
			if (!question.classList.contains('line_faq_active')) {
				...
				lineFaqText.forEach(text => {text.style.color = '#fff'});
				...
				hideLineFaqItem(lineFaqAnswer, lineFaqQuestion, serviceBtn);
				lineFaqText.forEach(text => {text.style.color = '#000'});
			}
		}
...
const whatNeedItems = Array.from(document.querySelectorAll(".wpaer_buy_num .item_num"));
if(whatNeedItems){
...
const messengerItems = Array.from(document.querySelectorAll(".position_rel .telegram, .position_rel .watsup"))
// Кнопка "вверх"
...
function initMap() {
	...
	let uluru = {};
	if (window.screen.width > 992) {uluru = { lat: 55.6822, lng: 37.6156 }};
...
function actionForm(form){
	...
	if(formPhone){
		let formPhonePlaceholder = formPhone.getAttribute('placeholder').replace(/x/g, 9);
	...
	if(!(formPhone)){formPhone = 1};
	// Согласине с политикой
	let formCheckbox = form.querySelector('#form__checkbox').checked;
	// Проверяем заполнение всех полей 
	let currValue = formCheckbox && (formName.value.length >=3)  && (formPhone.value.length > 0) && (formEmail.value.length >=6 && formEmail.value.includes('@')  && formEmail.value.includes('.'));
...
if(pageMuteBtn){
	...
	pageMuteBtn.onclick= function(){
		if (player.isMuted()) {
			player.unMute();
			player.setVolume(5);
			pageMuteBtn.innerHTML = '<img src="img/about/sound.svg" alt="" class="sound">'
		} else {
			pageMuteBtn.innerHTML = '<img src="img/about/muted.svg" alt="" class="muted">'
			player.mute();
		}
	}
}
...
if(sliderSteps){
    $(document).ready(function(){
		...
		$(".slider-steps").on("beforeChange", function(event, slick, currentSlide, nextSlide){
